// TESTE 02 - inserção consecutiva de elementos no final do vetor com alocação dinâmica
...
void test_push_back() { // Função que testa a inserção no final do vetor
    ...
    std::cout << "Inserção no final com " << NUM_ELEMENTS << " elementos: " << duration.count() << " ms" << std::endl; // Exibe o tempo de execução
}

int main() {
    test_push_back(); // Chama a função de teste para inserção no final
